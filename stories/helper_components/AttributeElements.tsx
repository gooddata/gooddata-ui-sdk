...
const attributeElementsProps: IAttributeElementsProps = {
    ...
    children: ({ validElements, loadMore, isLoading, error }: any) => {
        const { offset = null as any, count = null as any, total = null as any } = validElements
        ...
        return (
            <div>
                ...
                <button onClick={loadMore as any} disabled={isLoading || offset + count === total}>
                    More
                </button>
                <h2>validElements</h2>
                <p>
                ...
                <div>
                    {validElements
                        ? validElements.items.map((item: any) => (
                              <span key={item.element.uri} style={{ float: "left" }}>
                                  <AttributeFilterItem
                                      item={{
                                          selected: false,
...
storiesOf("Helper components/AttributeElements", module).add("with uri", () => (
    <div style={{ minHeight: 500 }}>
        <AttributeElements {...attributeElementsProps} />
    </div>
));
