...
test('should filter components according to selected employee', async (t) => {
    ...
    await t
        .expect(displayedNameSelector.textContent).eql('Aaron Clements')
        .expect(kpiMetricSelector.nth(0).textContent).eql('$129')
        .expect(kpiMetricSelector.nth(1).textContent).eql('$49')
        .click(listItemSelector.withText('Alex Meyer'))
        .expect(displayedNameSelector.textContent).eql('Alex Meyer')
        .expect(kpiMetricSelector.nth(0).textContent).eql('$157')
        .expect(kpiMetricSelector.nth(1).textContent).eql('$25');
});
