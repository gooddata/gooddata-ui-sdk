...
test('should display sorted graphs', async (t) => {
    ...
    await t
        // measure sorting
        .expect(measureSortingAxisLabels.textContent).eql('JanAprJunJulSepFebNovOctMarMayAugDec')
        ...
        .expect(dynamicSortingChartLegendLabels.textContent).eql('TexasNew YorkFloridaCaliforniaAlabama')

        // dynamic date sorting
        .click(dynamicSortingDate)
        .expect(dynamicSortingChartXAxisLabels.textContent).eql('DecNovOctSepAugJulJunMayAprMarFebJan')

        // dynamic sum of column sorting
        .click(dynamicSortingSumOfColumn)
        .expect(dynamicSortingChartXAxisLabels.textContent).eql('JanAprJunJulSepFebNovOctMarMayAugDec')

        // dynamic sum of stacks sorting
        .click(dynamicSortingSumOfStacks)
        .expect(dynamicSortingChartLegendLabels.textContent).eql('CaliforniaAlabamaFloridaNew YorkTexas')
        ...
        .expect(dynamicSortingChartLegendLabels.textContent).eql('TexasNew YorkFloridaAlabamaCalifornia')

        // dynamic multi sorting
        .click(dynamicSortingMulti)
        .expect(dynamicSortingChartXAxisLabels.textContent).eql('DecAugMayMarOctNovFebSepJulJunAprJan')
        .expect(dynamicSortingChartLegendLabels.textContent).eql('CaliforniaAlabamaFloridaNew YorkTexas')
