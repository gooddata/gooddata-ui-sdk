...
fixture('Pivot Table Demo') // eslint-disable-line no-undef
    .page(config.url)
    .beforeEach(loginUsingGreyPages(`${config.url}/pivot-table-demo`));
...
test('should render all tables', async (t) => {
    ...
    await t
        .expect(tableTotals.exists).ok();
    await t
        .expect(tableDrilling.exists).ok();
    await t
        .click(tableDrillingCell);
    await t
        .expect(drillValue.textContent).eql('Alcoholic Beverages');
});
