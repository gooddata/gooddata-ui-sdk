...
test('should be able to set from and to dates', async (t) => {
    ...
    await t
        .expect(xAxisLabels.textContent).eql('JanFebMarAprMayJunJulAugSepOctNovDec');

    await t
        .click(datePickerFrom)
        .click(datePickerFromNext)
        .click(datePickerFromDay1)
        .expect(xAxisLabels.textContent).eql('FebMarAprMayJunJulAugSepOctNovDec');

    await t
        .click(datePickerTo)
        .click(datePickerToPrevious)
        .click(datePickerToDay1)
        .expect(xAxisLabels.textContent).eql('FebMarAprMayJunJulAugSepOct');
});
