...
test('should be able to set measures and render them', async (t) => {
    ...
    await t
        .expect(sidebarItems.count).eql(4)
        .expect(lineSeries.count).eql(4)
        .expect(columnSeries.count).eql(4);

    await t
        .click(sidebarItems.nth(0))
        .click(sidebarItems.nth(1))
        .click(sidebarItems.nth(2))
        .expect(lineSeries.count).eql(1)
        .expect(columnSeries.count).eql(1);
});
