...
test('should filter table and chart and display employee info', async (t) => {
    await t
        ...
        .expect(salesBars.count).eql(1)
        .expect(employeeTableCells.nth(0).textContent).eql('Allen Garza');

    await t
        .click(employeeHeadingClearState) // clear
        .expect(salesBars.count).eql(12)
        .expect(employeeTableCells.nth(0).textContent).eql('Aaron Clements');

    await t
        .click(stateTableCells.nth(2)) // California
        .expect(salesBars.count).eql(5)
        .expect(employeeTableCells.nth(0).textContent).eql('Alex Meyer');
    ...
    await t
        ...
        .expect(salesBars.count).eql(1)
        .expect(employeeTableCells.nth(0).textContent).eql('Audrey Diamond');
    ...
    await t
        ...
        .expect(salesBars.count).eql(1)
        .expect(employeeTableCells.nth(0).textContent).eql('Anna Spencer');
    ...
    await t
        ...
        .expect(employeeProfile.exists).ok()
        .expect(employeeName.textContent).eql('Aaron Clements');
});
