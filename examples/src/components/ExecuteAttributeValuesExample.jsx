...
import { projectId, locationStateDisplayFormIdentifier, locationNameDisplayFormIdentifier } from '../utils/fixtures';
        ...
        return (
            <div>
                <ul className="attribute-values s-execute-attribute-values">
                    {result.executionResult.headerItems[0][0].reduce((buffer, state, stateIndex, states) => {
                        const { elements } = buffer;
                        let { values } = buffer;
                        values.push(result.executionResult.headerItems[0][1][stateIndex].attributeHeaderItem);
                        const isLastItem =
                            stateIndex === states.length - 1
                            || state.attributeHeaderItem.uri !== states[stateIndex + 1].attributeHeaderItem.uri;
                        if (isLastItem) {
                            elements.push(
                                <li key={state.attributeHeaderItem.uri}>
                                    <strong>{state.attributeHeaderItem.name}</strong>
                                    <ul key={state.attributeHeaderItem.uri}>
                                        {values.map(nameValue => (<li key={nameValue.uri}>{nameValue.name}</li>))}
                                    </ul>
                                </li>
                            );
                            values = [];
