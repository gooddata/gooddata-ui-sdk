...
function load(props: VisualisationConfig<DefaultProperties>) {
    return loadConfig(props, (key, value) => {
        switch (key) {
            case "colorMapping":
                return [["color", loadColorDefinitions(value as (typeof DEFAULTS)["colorMapping"])]];
            case "dataLabels": {
            ...
            case "orientation": {
                const val = value as (typeof DEFAULTS)["orientation"];
                return [["orientation", getValueOrDefault(val.position, DEFAULTS.orientation.position)]];
            }
            case "total": {
                const val = value as (typeof DEFAULTS)["total"];
                return [
                    ["total_enabled", getValueOrDefault(val.enabled, DEFAULTS.total.enabled, "bool")],
                    ["total_name", getValueOrDefault(val.name, DEFAULTS.total.name)],
                ];
            }
            case "legend": {
