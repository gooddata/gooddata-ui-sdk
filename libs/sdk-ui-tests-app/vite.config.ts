...
/**
 * Injects a <script src="./config.js"></script> tag into the built HTML in production mode.
 * This allows runtime configuration (e.g. WORKSPACE_ID) to be provided via a config.js file
 * generated before packaging, instead of being baked into the bundle at build time.
 */
...
function getDevelopmentAliases() {
    return [
        // This is required to make fonts work
        {
            ...
            replacement: path.resolve(__dirname, "../sdk-ui-kit/src/@ui"),
        },
...
export default defineConfig(({ mode }) => {
    ...
    return {
        ...
        define: {
            ...
            // TODO: BUILTIN_MAPBOX_TOKEN seems to be unused, no references found in src/
        ...
        server: {
            ...
            configure: (app: Connect.Server) => {
                ...
                app.use(
                    "/gooddata-ui-sdk",
                    (req: Connect.IncomingMessage, _res: unknown, next: Connect.NextFunction) => {
