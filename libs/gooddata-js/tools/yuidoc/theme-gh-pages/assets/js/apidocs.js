...
pjax.initClassTabView = function () {
    ...
    Y.all('.api-options').each(function(entry) {
        entry.all('input').each(function(input) {
            var id = Y.stamp(input);
            input.setAttribute('id', id);
            input.get('parentNode').setAttribute('for', id);
        });
        entry.delegate('click', pjax.onOptionClick, 'input');
    });
...
pjax.updateVisibility = function () {
    ...
    var node = Y.one(selectedTab.get('panelNode')) || Y.one("#classdocs");

    container.toggleClass('hide-inherited',
            !node.one('.api-show-inherited').get('checked'));

    container.toggleClass('show-deprecated',
            node.one('.api-show-deprecated').get('checked'));

    container.toggleClass('show-protected',
            node.one('.api-show-protected').get('checked'));

    container.toggleClass('show-private',
            node.one('.api-show-private').get('checked'));
...
pjax.onOptionClick = function (e) {
    ...
    store.set('options-inherited', node.one('.api-show-inherited').get('checked'));
    store.set('options-deprecated', node.one('.api-show-deprecated').get('checked'));
    store.set('options-protected', node.one('.api-show-protected').get('checked'));
    store.set('options-private', node.one('.api-show-private').get('checked'));
...
pjax.onTabSelectionChange = function (e) {
    ...
    var panelNode = selectedTab.get('panelNode');
    panelNode.one('.api-show-inherited').set('checked', store.get('options-inherited'));
    panelNode.one('.api-show-deprecated').set('checked', store.get('options-deprecated'));
    panelNode.one('.api-show-protected').set('checked', store.get('options-protected'));
    panelNode.one('.api-show-private').set('checked', store.get('options-private'));
