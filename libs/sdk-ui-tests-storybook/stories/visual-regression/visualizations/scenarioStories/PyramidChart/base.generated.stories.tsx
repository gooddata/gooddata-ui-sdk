...
export default {
    title: "01 Stories From Test Scenarios/PyramidChart/base",
};

export const SingleMeasure = () =>
    (() => {
        const scenarios = getScenariosGroupByIndexes(8, 0).asScenarioDescAndScenario();
...
export const SingleMeasureWithViewbySortedByAttribute = () =>
    (() => {
        ...
        const scenarioAndDescriptions = scenarios.filter(
            ([name]) => name === "single measure with viewBy sorted by attribute",
        );
        if (scenarioAndDescriptions.length === 0)
            throw new Error("Failed to find scenario 'single measure with viewBy sorted by attribute'");
        if (scenarioAndDescriptions.length > 1)
            throw new Error("Multiple 'single measure with viewBy sorted by attribute' scenarios found");
        const scenarioAndDescription = scenarioAndDescriptions[0];
...
export const SingleMeasureWithViewbySortedByMeasure = () =>
    (() => {
        const scenarios = getScenariosGroupByIndexes(8, 0).asScenarioDescAndScenario();
        const scenarioAndDescriptions = scenarios.filter(
            ([name]) => name === "single measure with viewBy sorted by measure",
        );
        if (scenarioAndDescriptions.length === 0)
            throw new Error("Failed to find scenario 'single measure with viewBy sorted by measure'");
        if (scenarioAndDescriptions.length > 1)
            throw new Error("Multiple 'single measure with viewBy sorted by measure' scenarios found");
        const scenarioAndDescription = scenarioAndDescriptions[0];
