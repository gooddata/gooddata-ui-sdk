...
export default {
    title: "01 Stories From Test Scenarios/ColumnChart/base",
};

export const SingleMeasure = () =>
    (() => {
        const scenarios = getScenariosGroupByIndexes(4, 0).asScenarioDescAndScenario();
...
export const SingleRatioMeasureWithViewby = () =>
    (() => {
        ...
        const scenarioAndDescriptions = scenarios.filter(
            ([name]) => name === "single ratio measure with viewBy",
        );
        if (scenarioAndDescriptions.length === 0)
            throw new Error("Failed to find scenario 'single ratio measure with viewBy'");
        if (scenarioAndDescriptions.length > 1)
            throw new Error("Multiple 'single ratio measure with viewBy' scenarios found");
        const scenarioAndDescription = scenarioAndDescriptions[0];
...
export const SingleMeasureWithViewbyAndStackbyFilteredToSingleStack = () =>
    (() => {
        const scenarios = getScenariosGroupByIndexes(4, 0).asScenarioDescAndScenario();
        const scenarioAndDescriptions = scenarios.filter(
            ([name]) => name === "single measure with viewBy and stackBy filtered to single stack",
        );
        if (scenarioAndDescriptions.length === 0)
            throw new Error(
                "Failed to find scenario 'single measure with viewBy and stackBy filtered to single stack'",
            );
        if (scenarioAndDescriptions.length > 1)
            throw new Error(
                "Multiple 'single measure with viewBy and stackBy filtered to single stack' scenarios found",
            );
        const scenarioAndDescription = scenarioAndDescriptions[0];
...
export const SingleMeasureWithOneViewbyAndOneStackbyWithMultipleItems = () =>
    (() => {
        const scenarios = getScenariosGroupByIndexes(4, 0).asScenarioDescAndScenario();
        const scenarioAndDescriptions = scenarios.filter(
            ([name]) => name === "single measure with one viewBy and one stackBy with multiple items",
        );
        if (scenarioAndDescriptions.length === 0)
            throw new Error(
                "Failed to find scenario 'single measure with one viewBy and one stackBy with multiple items'",
            );
        if (scenarioAndDescriptions.length > 1)
            throw new Error(
                "Multiple 'single measure with one viewBy and one stackBy with multiple items' scenarios found",
            );
        const scenarioAndDescription = scenarioAndDescriptions[0];
...
export const DenseChartWithTwoViewBy = () =>
    (() => {
        const scenarios = getScenariosGroupByIndexes(4, 0).asScenarioDescAndScenario();
        const scenarioAndDescriptions = scenarios.filter(([name]) => name === "dense chart with two view by");
        if (scenarioAndDescriptions.length === 0)
            throw new Error("Failed to find scenario 'dense chart with two view by'");
        if (scenarioAndDescriptions.length > 1)
            throw new Error("Multiple 'dense chart with two view by' scenarios found");
        const scenarioAndDescription = scenarioAndDescriptions[0];
