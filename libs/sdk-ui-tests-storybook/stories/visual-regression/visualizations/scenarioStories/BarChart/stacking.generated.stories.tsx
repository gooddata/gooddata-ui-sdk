...
export default {
    title: "01 Stories From Test Scenarios/BarChart/stacking",
};

export const TwoMeasuresAndTwoViewbyWithStackmeasures = () =>
    (() => {
        const scenarios = getScenariosGroupByIndexes(1, 24).asScenarioDescAndScenario();
...
export const TwoMeasuresAndTwoViewbyWithTopAxisAndStackmeasurestopercent = () =>
    (() => {
        const scenarios = getScenariosGroupByIndexes(1, 24).asScenarioDescAndScenario();
        const scenarioAndDescriptions = scenarios.filter(
            ([name]) => name === "two measures and two viewBy with top axis and stackMeasuresToPercent",
        );
        if (scenarioAndDescriptions.length === 0)
            throw new Error(
                "Failed to find scenario 'two measures and two viewBy with top axis and stackMeasuresToPercent'",
            );
        if (scenarioAndDescriptions.length > 1)
            throw new Error(
                "Multiple 'two measures and two viewBy with top axis and stackMeasuresToPercent' scenarios found",
            );
        const scenarioAndDescription = scenarioAndDescriptions[0];
