...
export default {
    title: "01 Stories From Test Scenarios/WaterfallChart/customization/theme",
};

export const Themed = () =>
    (() => {
        const scenarios = getScenariosGroupByIndexes(18, 15).asScenarioDescAndScenario();
...
export const MultiMeasuresWithThemed = () =>
    (() => {
        const scenarios = getScenariosGroupByIndexes(18, 15).asScenarioDescAndScenario();
        const scenarioAndDescriptions = scenarios.filter(([name]) => name === "multi measures with themed");
        if (scenarioAndDescriptions.length === 0)
            throw new Error("Failed to find scenario 'multi measures with themed'");
        if (scenarioAndDescriptions.length > 1)
            throw new Error("Multiple 'multi measures with themed' scenarios found");
        const scenarioAndDescription = scenarioAndDescriptions[0];
...
export const MultiMeasuresWithFont = () =>
    (() => {
        const scenarios = getScenariosGroupByIndexes(18, 15).asScenarioDescAndScenario();
        const scenarioAndDescriptions = scenarios.filter(([name]) => name === "multi measures with font");
        if (scenarioAndDescriptions.length === 0)
            throw new Error("Failed to find scenario 'multi measures with font'");
        if (scenarioAndDescriptions.length > 1)
            throw new Error("Multiple 'multi measures with font' scenarios found");
        const scenarioAndDescription = scenarioAndDescriptions[0];
