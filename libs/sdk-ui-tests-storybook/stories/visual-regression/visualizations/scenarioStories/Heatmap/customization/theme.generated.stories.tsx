...
export default {
    title: "01 Stories From Test Scenarios/Heatmap/customization/theme",
};

export const Themed = () =>
    (() => {
        const scenarios = getScenariosGroupByIndexes(10, 8).asScenarioDescAndScenario();
...
export const ThemedWithNullValues = () =>
    (() => {
        const scenarios = getScenariosGroupByIndexes(10, 8).asScenarioDescAndScenario();
        const scenarioAndDescriptions = scenarios.filter(([name]) => name === "themed with null values");
        if (scenarioAndDescriptions.length === 0)
            throw new Error("Failed to find scenario 'themed with null values'");
        if (scenarioAndDescriptions.length > 1)
            throw new Error("Multiple 'themed with null values' scenarios found");
        const scenarioAndDescription = scenarioAndDescriptions[0];
