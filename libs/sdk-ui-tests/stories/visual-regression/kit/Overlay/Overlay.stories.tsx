...
function OverlayExample({ index = 0 }: IOverlayExampleProps) {
    const intl = useIntl();
    ...
    const buttonText = useCallback(() => {
        ...
        return isOpen ? `${closeOverlay}${index}` : `${openOverlay}${index}`;
    }, [index, intl, isOpen]);
        ...
        return isOpen ? (
            <div className="overlay-wrapper">
                <MediaQuery query="only screen and (min-width:640px)">
                    <Overlay
                        alignTo={`.overlay-example-opener-${index}`}
                        alignPoints={[
                            {
                                align: "bl tl",
                                offset: {
    ...
    }, [index, isOpen, buttonText, toggleOverlay]);

    return (
        <div className="overlay-example">
            <Button
                className={`gd-button-primary overlay-example-opener-${index}`}
                value={buttonText()}
                onClick={toggleOverlay}
            />
            {renderOverlay()}
        ...
        >
            <div className="library-component screenshot-target">
                <div style={{ float: "right" }}>
                    <OverlayExample index={2} />
                </div>
                <OverlayExample index={1} />
                <p>
