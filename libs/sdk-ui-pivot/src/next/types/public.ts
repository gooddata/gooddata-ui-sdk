 ...
 *
 * @remarks
 * `PivotTableNextConditionalFormattingConfig` is intentionally NOT folded in while conditional
 * formatting is `@alpha` — a `@public` type cannot reference an `@alpha` one (api-extractor
 * `ae-incompatible-release-tags`). Use `PivotTableNextConfigWithConditionalFormatting` for the
 * combined shape until then.
 *
 * @public
 */
