...
function getRowDataFont(
    ...
    if (
        isColumnTotal(colDef) ||
        (isScopeCol(colDesc) && colDesc.isTotal) ||
        (isMixedValuesCol(colDesc) && colDesc.isTotal)
    ) {
        return config.totalColumnFont;
    } else if (
        isColumnSubtotal(colDef) ||
        (isScopeCol(colDesc) && colDesc.isSubtotal) ||
        (isMixedValuesCol(colDesc) && colDesc.isSubtotal)
    ) {
        return config.subtotalColumnFont;
