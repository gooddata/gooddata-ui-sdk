...
export const formattingLanguage = StreamLanguage.define<LanguageState>({
    ...
    token(stream, state) {
        ...
        if (state.isInArithmeticExpression || stream.match(/^\{\{\{(?:\d+(?:\.\d+)?)?\|(?:\d+\.?)?\|[^}]+\}\}\}/, false)) {
            if (stream.match("{{{")) {
        ...
        // Colors
        if (state.isInColor || stream.match(/^\[(?:black|blue|cyan|green|magenta|red|white|yellow|none)\]/i, false)) {
            if (stream.match("[")) {
        ...
        // Custom colors
        if (state.isInCustomColor || stream.match(/^\[(?:backgroundColor|color)=#?(?:[a-f0-9]{6}|[a-f0-9]{3})\]/i, false)) {
            if (stream.match("[")) {
