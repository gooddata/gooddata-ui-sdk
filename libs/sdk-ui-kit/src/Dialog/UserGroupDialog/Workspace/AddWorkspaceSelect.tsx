 ...
 * @internal
 */
export const AddWorkspaceSelect: React.FC<IAddWorkspaceSelectProps> = ({ addedWorkspaces, grantedWorkspaces, onSelectWorkspace }) => {
    ...
    const noOptionsMessage = useMemo(
        () => () => intl.formatMessage({ id: "userGroupDialog.workspace.noMatchingItems" }),
        [intl],
    );

    const loadOptions = useMemo(
        () => debounce(
            loadWorkspaceOptionsPromise(backend, intl),
            SEARCH_INTERVAL,
            { leading: true },
        ),
        [backend, intl],
    ...
    const usedWorkspace = useMemo(() => ([...addedWorkspaces, ...grantedWorkspaces]),[addedWorkspaces, grantedWorkspaces])
        ...
        <div className="gd-share-dialog-content-select">
            <AsyncSelect
                ...
                placeholder={intl.formatMessage({
                    id: "userGroupDialog.workspace.searchPlaceholder",
                })}
