 ...
 * @internal
 */
export function useUiMenuContextValue<InteractiveItemData, StaticItemData>(
    props: UiMenuProps<InteractiveItemData, StaticItemData>,
    menuComponentRef: React.RefObject<HTMLElement>,
    itemsContainerRef: React.RefObject<HTMLElement>,
): IUiMenuContext<InteractiveItemData, StaticItemData> {
    const {
        items,
    ...
    const makeItemId = React.useCallback<IUiMenuContext<InteractiveItemData, StaticItemData>["makeItemId"]>(
        (item) => (item && item.type === "interactive" ? `item-${ariaAttributes.id}-${item.id}` : uuid()),
        [ariaAttributes.id],
    );

    const scrollToView = React.useCallback<
        IUiMenuContext<InteractiveItemData, StaticItemData>["scrollToView"]
    >(
        (element) => {
            if (!element) {
                return;
...
const ItemComponent = React.memo(function ItemComponent<InteractiveItemData, StaticItemData>({
    item,
}: UiMenuItemProps<InteractiveItemData, StaticItemData>) {
...
export function useKeyNavigation<InteractiveItemData, StaticItemData>({
    ...
    menuContextValue: IUiMenuContext<InteractiveItemData, StaticItemData>;
