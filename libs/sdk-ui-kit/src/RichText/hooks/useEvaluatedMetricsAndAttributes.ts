 ...
 *
 * A computed attribute is referenced exactly like any other attribute - its ref goes on the display
 * form slot, typed `computedAttribute` - and is aggregated the same way. Where the backend cannot
 * resolve the context an aggregation over one needs, it rejects the execution and every reference
 * in the widget renders as unretrievable; see {@link useEvaluatedMetricsAndAttributes}.
 *
 * @internal
 */
