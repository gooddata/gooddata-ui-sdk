...
Thu, 10 Sep 2026 06:58:30 GMT

### Minor changes

- sdk-backend-spi: Add computedAttributes to IComputedAttributeReferencing, reporting other computed attributes that reference a computed attribute in their MAQL
- sdk-backend-base: search small attribute element lists locally instead of asking the backend.

### Patches

- sdk-backend-tiger: Include other computed attributes referencing a computed attribute in getComputedAttributeReferencingObjects
- sdk-ui-catalog: Warn about referencing computed attributes, not just insights/metrics/dashboards, before deleting a computed attribute
- sdk-ui-catalog: Enable the Lineage tab for computed attributes in the Analytics Catalog, and stop collapsing computed-attribute nodes out of other objects' lineage graphs
- sdk-ui-filters: Add isAbsoluteDateFilterGranularityEnabled prop to DateFilter for granularity switching (with localized quarter labels) in the static period form; corrects stale/unavailable granularities
- sdk-ui-gen-ai: Add `displayMode` for GenAI components.
- sdk-code-schemas, sdk-code-convertors: Numeric parameters can be written as code (type NUMBER with defaultValue and min/max constraints); clone and deploy carry them like textual ones
- sdk-code-schemas, sdk-code-convertors: A computed attribute written as code can be referenced as computed_attribute/{id} from visualization fields, attribute, text and ranking filters, metric value filter dimensionality, dashboard filters and drills to URL
- sdk-code-schemas: Computed attribute locale is validated against the shared BCP 47 pattern instead of accepting any string
- sdk-ui-catalog: The computed attribute YAML editor accepts, shows and saves format, metric_type, data_type, value_type, is_nullable, null_value_join_replacement and show_in_ai_results, validates locale as BCP 47, names the offending field for enum and locale errors, and keeps the value type when duplicating
- sdk-model, sdk-backend-tiger: Computed attribute carries valueType; the fabricated display form derives its displayFormType from it, so hyperlink and image computed attributes render like labels of that type
- sdk-code-schemas, sdk-code-convertors: Computed attribute as code carries data_type, value_type, format, metric_type, is_nullable, null_value_join_replacement and show_in_ai_results, so deploy no longer overrides or drops them
- sdk-backend-spi: Export DefaultWorkspaceCatalogTypes, the catalog item types loaded when forTypes does not narrow them; the tiger catalog factories reuse it as their default
- sdk-ui-dashboard: Unified automation dialog lifecycle callbacks (breaking, all affected symbols @alpha): onCreateSuccess/onCreateError/onUpdateSuccess/onUpdateError replace onSuccess/onError/onSaveSuccess/onSaveError; scheduled email onSubmit/onSave observers removed; management dialog delete/pause callbacks removed; useDashboardAlerts/useDashboardScheduledEmails Save members renamed to Update.
- sdk-ui-dashboard: The alerting and scheduled email dialog context decorator seams now warn in the console (once per dialog open) when a decorator forces isLoading false while the dialog's data still loads or drops context keys the connector provided, instead of failing silently downstream.
- Fix: a scheduled export save no longer starts twice when submitted again while a save is in flight.
- sdk-ui-dashboard: The automation dialog state providers now keep the dialog state model mounted once the dialog's data has first loaded, so an automations refresh under an open alerting or scheduled email dialog no longer discards the in-flight draft (in the default dialogs and in custom replacements).
- sdk-ui-kit: The workspace catalog header item always links to the hosted /workspace/{id}/catalog route
- sdk-model, sdk-backend-tiger: Remove the enableShellApplication_catalog feature flag; the catalog is always served by the host application
- sdk-ui-pluggable-host: mapLegacyUrlToHost also maps the legacy standalone home-ui paths (/workspaces, /data-sources, /users-and-groups, /settings, /configuration, /automations, /ai-hub, /getting-started) onto the host, resolving the /settings#ai AI Hub deeplink to the AI Hub route

### Updates

- sdk-ui-dashboard: Pass through absolute date filter granularity data, seed its baseline from the feature flag at dashboard initialization, and let dateFilterConfig.hideGranularities restrict the absolute (static) date filter
- sdk-ui-gen-ai: Introduce clarifying questions support in GenAI conversations.
- sdk-ui-all: Enhance label matching logic, dataset references in dashboard copilot.
- sdk-ui-gen-ai: Update provider resolution and vis mapping logic, client update.
- sdk-ui-kit: Added bold and italic icons.
- sdk-ui-kit: Added the RichText allowedMarkdown prop, which narrows the recognised markdown to a chosen set of features.
- sdk-backend-tiger: Turn the enableConditionalFormatting feature flag on by default
- sdk-ui-pluggable-host: Showed the contract-expired lock screen when the backend denies requests because the organization's contract or the deployment license expired, instead of a generic "Failed to load" page naming the tier.
- sdk-backend-tiger: Recognized a deployment-wide license expiry (`license_expired`) as a contract-expired error alongside `contract_expired`, instead of surfacing it as an unexpected error on login.
- sdk-ui-all: Sort computed attributes alphabetically among attributes in the metric attribute filter and measure value filter dimensionality pickers.
- Computed attributes can now be used on Dashboards: they are loaded into the dashboard catalog and work in attribute filters (including parent filters), drill interactions, cross-filtering, Key Driver Analysis and measure value filters; their detail tooltips show the values and the MAQL definition.
- sdk-ui-all: Allow computed attributes in insight metric buckets.
- sdk-ui-all: Keep the computed attribute type of a catalog computed attribute used in the measure value filter dimensionality in Analytical Designer, so the backend no longer resolves it as a label.
- sdk-backend-tiger: The label elements request now states whether it targets a label or a computed attribute, so an id shared by both no longer resolves ambiguously.
- gdc-home-ui: Restricted the clickable area of the "Allow users to change timezone" toggle in the timezone settings dialog to the switch itself instead of the whole row, matching the standard toggle behavior across the app.
- sdk-ui-catalog: Computed attributes can be shared from the catalog detail (object-level permissions).
- sdk-ui-ext: Fix useObjectAccessLevel matching the caller's own grant, which never hit because the profile ref is a URI while access list grants carry the user id.
- sdk-ui-ext: Add the selfGranteeRef and summaryOtherGranteeCount helpers, so a consumer fetching its own access list can discount the caller's grant the same way the share dialog does.
- sdk-backend-tiger: Read forbidden dashboard references from the metadata API meta.restricted signal while retaining deleted-reference detection.
- sdk-ui-gen-ai: Create a reset conversation with the agent selected in the picker, so the agent's personality applies to the first message instead of only after an explicit switch.
- sdk-ui-ext: Add draftToSummary and summaryToShareLevel, so a consumer can show an object's access level without opening the share dialog.
- sdk-ui-ext: Show the caller as the creator in the object share dialog's draft mode, so access for an object that does not exist yet is not presented as an empty list.
- sdk-ui-ext: Reword the object share dialog's confirmation toasts to match the object-level permissions design.
- sdk-ui-kit: Show the inherited-workspace-access explanation as a tooltip on the disabled Restricted general-access option instead of a permanent inline description.
- sdk-ui-ext: Add the useObjectAccessLevel hook, which reads an object's access level for display outside the share dialog.
- sdk-ui-all: Dashboard insight widgets the user may not see now render an access placeholder instead of the visualization.
- sdk-ui-all: A visualization switcher entry whose visualization the user cannot access stays in the list, labelled as restricted, and shows an access placeholder instead of an empty widget.
- chore: Upgrade react-intl to 10.1.25
- maql-language-server: Resolve the parameters a visualisation as code depends on — through inline MAQL and referenced metrics — to the default values it executes with, and show them in the visualisation preview
- sdk-ui-all: Allow attribute filters based on computed attributes.
- api-client-tiger: Regenerate the client from the latest metadata and gen-ai OpenAPI specs. The report, reportTemplate and reportPageLayout entities gain createdAt and modifiedAt attributes, createdBy and modifiedBy relationships and an include parameter on their CRUD endpoints.
- sdk-model: Add the dashboard_builder and dashboard_editor values to AgentCustomSkill, keeping it in sync with the backend skill enum.
- sdk-backend-tiger: Drop the new clarifyingQuestions chat part instead of failing the chat item, as it has no model representation yet.
- sdk-backend-tiger: Populate created, updated, createdBy and updatedBy on the reports, report templates and report page layouts returned by IWorkspaceReportsService, requesting the createdBy and modifiedBy relationships on every read and write.
- sdk-ui-dashboard: Gate the edit-mode left panel on enableGenAiRightPanel and focus the search when the rail expands it.
- sdk-ui-dashboard: Refresh the edit-mode left panel including collapse/expand chrome and side-panel icons.
- sdk-ui-catalog: Metric detail gains a Conditional Formatting row for authoring semantic-layer rules (behind enableSemanticConditionalFormatting).
- sdk-ui-ext: the share dialog shows the administrator's display name instead of the raw user id.
- sdk-ui-ext: share dialog explains disabled levels with "You can't set permissions higher than your own."
- sdk-ui-ext: the share dialog keeps showing a workspace administrator's own row when the object is shared with others.
...
- sdk-ui-all: Add support for computed attributes in model and AD.

### Patches

- sdk-model, sdk-backend-spi: Add reportPageLayout, reportTemplate and report model objects with built-in page layouts and the workspace reports SPI
- sdk-ui-filters: Add the new PeriodRangePicker date picker component (with a Storybook story per granularity), along with the supporting static date filter granularity types and period-boundary conversion utilities (not yet wired into DateFilter)
- sdk-model: Add the enableBusinessBriefingReportsApp feature flag
- sdk-ui: Stop reporting the cancellation of a superseded execution as an execution failure. withEntireDataView decided whether a response was still relevant by comparing execution definition fingerprints, which does not identify a request: when the same definition is requested again after a different one, an already-cancelled earlier request stops looking superseded and its AbortError is delivered through onError. Analytical Designer showed "Sorry, we can't display this visualization" over a chart that had loaded, whenever a saved insight carrying a sort was opened. The staleness check now keys on the request itself.

### Updates

- sdk-ui-dashboard: Reset parameters omitted by an applied filter view instead of keeping the previously applied value
- sdk-ui-all: Added data-activeapp attribute to the html tag.
- sdk-ui: Added an `Inherit` option to the CSV delimiter picker used in scheduled exports, alerts, and ad-hoc CSV exports. When selected (the new default when no delimiter is saved), no delimiter is sent and the workspace/organization default applies.
- sdk-backend-base: CacheControl now exposes resetAutomations to invalidate the workspace automations cache, and resetAll clears it together with the other caches.
- sdk-backend-tiger: Turn on enableSecondGranularities feature flag by default.
...
- sdk-ui-kit: Added the ISlotProps slot contract (alpha), moved from sdk-ui-dashboard
- sdk-ui-dashboard: ISlotProps is now re-exported from sdk-ui-kit; the sdk-ui-dashboard export is unchanged for consumers
- sdk-model, sdk-backend-tiger: Add the canCreateMetric workspace permission, resolved from the granular CREATE_METRIC backend permission.
- sdk-ui-kit: Fix the message component being squeezed below its own content when it is a flex item, which collapsed a message to a few pixels and clipped its text next to a scrollable list in a dialog.
- sdk-ui-gen-ai: Apply the workspace number separators to visualizations embedded in the AI chat, so they match the same insight in Analyze
- Add the setTimezone postMessage command for embedded dashboards
- sdk-model: Pin the @types/stringify-object dependency to the exact version 3.3.1 instead of the ^3.2.0 range.
- sdk-ui-kit: Pin the @types/tinycolor2 dependency to the exact version 1.4.6 instead of the ^1.4.2 range.
- sdk-ui-kit, sdk-ui-ext: Pin the cron-parser dependency to the exact version 5.9.0 instead of the ^5.1.1 range.
- sdk-ui, sdk-ui-charts, sdk-ui-filters, sdk-ui-pivot, sdk-ui-ext: Pin the full-icu dependency to the exact version 1.6.0 instead of the ^1.3.0 range.
- sdk-code-schemas: Pin the json-schema-to-typescript dependency to the exact version 13.1.2 instead of the ^13.0.1 range.
- sdk-ui-kit: Pin the kefir dependency to the exact version 3.8.8 instead of the ^3.8.8 range.
- api-client-tiger, i18n-toolkit: Pin the mkdirp dependency to the exact version 1.0.4 instead of the ^1.0.4 range.
- sdk-ui-gen-ai, sdk-ui-dashboard, sdk-ui-kit, sdk-ui-ext: Pin the svgo dependency to the exact version 2.8.3 instead of the ^2.8.0 range.
- sdk-ui: Pin the tsd dependency to the exact version 0.32.0 instead of the ^0.32.0 range.
- plugin-toolkit, app-toolkit: Pin the validate-npm-package-name dependency to the exact version 5.0.1 instead of the ^5.0.0 range.
- sdk-code-schemas, sdk-code-convertors: Add the `parameter` object type to analytics-as-code for textual parameters — `type: parameter` YAML with a `definition` of `type: STRING`, a default value and optional constraints (length bounds or a governed allowed-value list), plus `declarativeParameterToYaml` / `yamlParameterToDeclarative` so string parameters clone from and deploy to a workspace; metrics referencing `{parameter/id}` in MAQL now resolve, so the reference is validated, navigable, renameable, and protects the parameter from deletion. Parameters of other data types keep whatever created them and are left untouched by clone and deploy
- sdk-code-schemas, maql-language-server: Render a metric's MAQL template with its parameters' default values, so a template can be previewed as the MAQL that would execute, and reject string parameter definitions whose values contradict each other — a default outside the allowed values, a duplicated allowed value or title, bounds describing an empty range, or a default outside those bounds
- sdk-ui: reload chart data when only the execution buckets change, so charts deriving measure roles from buckets (Mekko Width/Height, scatter and bubble axes, bullet series) re-render when a lone measure moves between buckets
- sdk-ui-charts: fix Mekko chart data label color being inconsistent between AD and KD when points are drillable
- sdk-ui-ext: add zoom and pan support to Mekko chart
- sdk-ui-dashboard: ChangeFilterContextSelection accepts matchByLocalIdentifier to match attribute filters by localIdentifier first (display form fallback); filter view application and the reset filters button use it, so filters sharing a display form no longer clobber each other. changeFilterContextSelectionByParams is now public and the positional changeFilterContextSelection factory is deprecated in its favor.
- sdk-ui-gen-ai, sdk-ui-kit: Accessibility fixes for the assistant context — named chip group with spoken context changes, chip type and delete button names (UiChip can now name its leading icon)
- sdk-ui-gen-ai, sdk-model: The AI assistant context picker shows the visualization type icon for the offered visualizations and dashboard widgets.
- sdk-ui-gen-ai: A new AI assistant conversation starts in the Quick reasoning mode instead of Thinking.
- sdk-ui-charts: fix grouped category label resolving to an object instead of a string when the category's name is empty
- sdk-ui-gen-ai: Add dashboard select as prop for auto update ambient context.
- sdk-ui-gen-ai: Update design of references in ai assistant.
- sdk-ui-pluggable-host: Disable module app stylesheets on unmount so app CSS no longer bleeds across apps
- sdk-ui-gen-ai: Enhance reference handling and update UI for tooltips over them.
- sdk-ui-gen-ai: Introduce slot-based customization for Gen AI assistant.
- sdk-ui-all: Visual exports (PDF, PPTX) keep the filters applied on screen even when a default saved filter view exists
- sdk-ui-ext: the share dialog now disables permission levels that the user cannot grant, instead of letting the server refuse them.
