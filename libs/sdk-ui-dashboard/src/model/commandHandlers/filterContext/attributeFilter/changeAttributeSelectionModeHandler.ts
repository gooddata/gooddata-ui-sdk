...
import { type ISetAttributeFilterSelectionMode } from "../../../commands/filters.js";
...
export function* changeAttributeSelectionModeHandler(
    ...
    cmd: ISetAttributeFilterSelectionMode,
): SagaIterator<void> {
    ...
    const affectedFilter: ReturnType<ReturnType<typeof selectFilterContextAttributeFilterItemByLocalId>> =
        yield select(selectFilterContextAttributeFilterItemByLocalId(cmd.payload.filterLocalId));
    ...
    yield put(
        tabsActions.changeSelectionMode({
            filterLocalId,
            selectionMode,
        }),
    );
