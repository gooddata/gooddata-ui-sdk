...
import {
    ...
    type IUseAlertDraftFilterWritesProps,
} from "./useAlertDraftFilterWrites.js";
...
const BASE_PROPS: IUseAlertDraftFilterWritesProps = {
...
function renderDraftFilterWritesHook(props: Partial<IUseAlertDraftFilterWritesProps> = {}) {
    const mergedProps: IUseAlertDraftFilterWritesProps = { ...BASE_PROPS, ...props };
    return renderHook(() => useAlertDraftFilterWrites(mergedProps));
}
...
describe("useAlertDraftFilterWrites — selectedMeasure branch", () => {
    ...
    it("returns the transformAlertByAttribute output unchanged when selectedMeasure is undefined, no metric call", () => {
        const { result } = renderDraftFilterWritesHook({ selectedMeasure: undefined });
...
describe("useAlertDraftFilterWrites — !s.metadata?.widget flag", () => {
...
describe("useAlertDraftFilterWrites — undefined state", () => {
    it("returns undefined without throwing when the updater is applied to undefined", () => {
        const { result } = renderDraftFilterWritesHook();
...
describe("useAlertDraftFilterWrites — selectedValue nullish", () => {
    it("passes name: '' to transformAlertByAttribute when selectedValue is undefined", () => {
        const { result } = renderDraftFilterWritesHook({ selectedValue: undefined });
    ...
    it("passes name: '' to transformAlertByAttribute when selectedValue is null", () => {
        const { result } = renderDraftFilterWritesHook({ selectedValue: null });
