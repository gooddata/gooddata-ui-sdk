...
export interface IShareableLabels {
    /** Display forms of the shareable attribute; empty for every other shareable kind. */
    labels: IObjectShareLabel[];
    /**
     * True while an attribute's labels are still being fetched. The other shareable kinds
 ...
 * can scope a grantee's access per label. The other shareable kinds have no labels —
...
export function useShareableLabels(item: ShareableCatalogItem | undefined): IShareableLabels {
    ...
    // Only attributes carry labels; the other shareable kinds do not.
    ...
    // Only an attribute fetches, so only an attribute "loads": until its fetch settles (success or error).
