...
export interface IShareableLabels {
    /** Display forms of the shareable attribute; empty for facts, measures and computed attributes. */
    labels: IObjectShareLabel[];
    /**
     * True while an attribute's labels are still being fetched. Facts, measures and
     * computed attributes skip the fetch and report `false`. Gates the dialog's Add action so a grantee
 ...
 * can scope a grantee's access per label. Facts, measures and computed attributes
 * have no labels — returns an empty list and skips the fetch. The result feeds
...
export function useShareableLabels(item: ShareableCatalogItem | undefined): IShareableLabels {
    ...
    // Only attributes carry labels; facts, measures and computed attributes do not.
    ...
    // Facts, measures and computed attributes have no fetch, so they never "load". An attribute loads
