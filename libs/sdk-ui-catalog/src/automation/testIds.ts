// (C) 2025-2026 GoodData Corporation

/*
 * NOTE: Testing IDs should be considered as immutable to prevent breaking changes in E2E tests.
 */

export const catalog = "analytics-catalog";
export const objectType = `${catalog}-object-type`;
export const filter = `${catalog}-filter`;
export const filterCreatedBy = `${filter}/created-by`;
export const filterOrigin = `${filter}/origin`;
export const filterVisibility = `${filter}/visibility`;
export const filterCertification = `${filter}/certification`;
export const filterQuality = `${filter}/quality`;
export const filterTags = `${filter}/tags`;
export const filterReset = `${filter}/reset`;
export const catalogDetail = `${catalog}-detail`;
export const catalogDetailActionsTrigger = `${catalogDetail}-actions/trigger`;
export const catalogDetailActionDuplicate = `${catalogDetail}-actions/duplicate`;
export const catalogDetailActionDelete = `${catalogDetail}-actions/delete`;
export const catalogDetailActionShare = `${catalogDetail}-actions/share`;
export const catalogDetailAccessRow = `${catalogDetail}-access-row`;
export const catalogDetailAccessRowWorkspace = `${catalogDetail}-access-row/workspace`;
export const catalogDetailAccessRowShared = `${catalogDetail}-access-row/shared`;
export const catalogDetailAccessRowPrivate = `${catalogDetail}-access-row/private`;
export const catalogDetailLabels = `${catalogDetail}-labels`;
export const catalogDetailLabelsTrigger = `${catalogDetail}-labels/trigger`;
export const catalogDetailLabelsList = `${catalogDetail}-labels/list`;
