...
const timezones = [
    ...
    (t: [string, string, number, number]): ITimezone => ({
        identifier: t[0],
        title: t[1],
        januaryOffset: t[2],
        juneOffset: t[3],
...
const TIMEZONE_BY_OFFSETS = timezones.find((tz: ITimezone): boolean => {
    return tz.januaryOffset === JANUARY_OFFSET && tz.juneOffset === JUNE_OFFSET;
});

const TIMEZONE_DEFAULT = timezones.find((tz: ITimezone): boolean => {
