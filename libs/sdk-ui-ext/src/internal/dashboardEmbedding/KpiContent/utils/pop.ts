...
function getTrend(prev: number, curr: number): Trend {
...
function isTrendPositive(trend: Trend, meaning: string) {
...
function isTrendNegative(trend: Trend, meaning: string) {
...
function getMeaning(trend: Trend, meaning: string): Meaning {
    if (trend === "neutral") {
        return "neutral";
...
function getPercentageValue(prev = 0, curr = 0) {
    const previous = prev || 0;
    const current = curr || 0;
...
export function getPopInfo(previous: number, current: number, meaning: string): IPopInfo {
