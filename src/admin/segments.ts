...
export class SegmentsModule {
    ...
    public getDataProductSegments(contractId: string, dataProductId: string) {
        return this.xhr.get(interpolate(CONTRACT_DATA_PRODUCT_SEGMENTS, { contractId, dataProductId }))
            ...
            body: JSON.stringify({
                segmentCreate: {
                    ...
                    domains: domainIds.map(
                        (domainId: string) => interpolate(CONTRACT_DOMAIN, { contractId, domainId })
                    )
                }
            })
        });
    ...
    public deleteSegment(contractId: string, dataProductId: string, segmentId: string) {
        return this.xhr.del(interpolate(CONTRACT_DATA_PRODUCT_SEGMENT, { contractId, dataProductId, segmentId }));
    }
}
