...
export class DomainDataProductModule {
    ...
    public getDomainDataProducts(contractId: string, dataProductId: string) {
        return this.xhr.get(interpolate(CONTRACT_DATA_PRODUCT_DOMAIN_DATA_PRODUCTS, { contractId, dataProductId }))
            .then((result: any) => {
                const { domainDataProducts: { items }, status }: any = result.getData();
                return {
