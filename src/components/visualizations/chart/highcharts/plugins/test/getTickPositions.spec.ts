...
describe("get tick positions for dual axes chart", () => {
    ...
    describe("test generate min/max values", () => {
        ...
        const createMinMax = (left: IMinMax, right: IMinMax): IMinMaxInfo[] => {
            return [{ ...minmaxTemplate[0], ...left }, { ...minmaxTemplate[1], ...right }];
        };
        ...
        const compareMinMax = (actual: IMinMax[], expected: IMinMax[]) => {
            ...
            expected.forEach((item: IMinMax, axisIndex: number) =>
                assertMinMax(
                    item,
                    getMinMax(axisIndex, minmax[axisIndex].min, minmax[axisIndex].max, minmax),
                ),
            );
        };
    ...
    describe("test generate tick positions with highchart", () => {
        ...
        describe("test on dual axes", () => {
            ...
            const runTest = (leftAxisInfo: IAxisInfo, rightAxisInfo: IAxisInfo) => {
                const leftAxis = getAxis(
                    leftAxisInfo.index,
                    leftAxisInfo.dataMin,
                    leftAxisInfo.dataMax,
                    leftAxisInfo.userMin,
                    leftAxisInfo.userMax,
                );

                const rightAxis = getAxis(
                    rightAxisInfo.index,
                    rightAxisInfo.dataMin,
                    rightAxisInfo.dataMax,
                    rightAxisInfo.userMin,
                ...
                const leftTickPositions = getTickPositions.call(
                    leftAxis,
                    leftAxisInfo.userMin || leftAxis.min,
                    leftAxisInfo.userMax || leftAxis.max,
                );
                ...
                const rightTickPositions = getTickPositions.call(
                    rightAxis,
                    rightAxisInfo.userMin || rightAxis.min,
                    rightAxisInfo.userMax || rightAxis.max,
                );
    ...
    describe("createArrayFromRangeWithMiddleZero", () => {
        it.each([[-100000, 1000000, 12], [-0.2100000000000003, 2.1, 12]])(
            "should add zero between %f and %f values",
            (start: number, end: number, ticksNumber: number) => {
